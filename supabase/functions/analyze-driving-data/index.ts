...
serve(async (req) => {
  ...
  try {
    ...
    const hardBrakingCount =
      (text.match(/hard braking|sudden stop|emergency brake/g) || []).length;
    const rapidAccelerationCount =
      (text.match(/rapid acceleration|hard acceleration|quick start/g) || [])
        .length;
    const steadyDrivingCount =
      (text.match(/steady|constant speed|cruise control/g) || []).length;

    const aggressiveScore = hardBrakingCount + rapidAccelerationCount -
      steadyDrivingCount;
