...
serve(async (req) => {
  ...
  try {
    ...
    console.log(
      `Analyzing ${photoUrls.length} photos for valuation: ${valuationId}`,
    );
    ...
    try {
      ...
      if (
        individualScores && Array.isArray(individualScores) &&
 ...
 */
async function analyzeWithScores(
  scores: PhotoScore[],
  ...
  const avgScore = scores.reduce((sum, score) => sum + score.score, 0) /
    scores.length;
  ...
  const confidenceScore = Math.min(
    95,
    Math.max(30, 100 - (scoreVariance * 100) - (5 * (5 - scores.length))),
  );
 ...
 */
async function simulateConditionAssessment(
  photoUrls: string[],
): Promise<ConditionAssessment> {
  // Simulate API call delay
...
function generateConditionSummary(condition: string, issues: string[]): string {
  ...
  const baseSummaries = summaries[condition as keyof typeof summaries] ||
    summaries["Good"];
  const baseSummary =
