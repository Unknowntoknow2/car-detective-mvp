...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Parse request data
    const { session_id, user_input, valuation_id, new_session, user_context } =
      await req.json();
    ...
    if (!valId) {
      ...
      if (sessionError) {
        throw new Error(
          `Failed to fetch session data: ${sessionError.message}`,
        );
      }
    ...
    const systemPrompt =
      `You are Car Detective, a world-class AI assistant that helps car owners understand their valuation, improve their chances of a better deal, and take confident next steps.
...
IMPORTANT: When noticing signs of frustration, acknowledge the user's feelings and offer constructive solutions.`;
    ...
    if (valuationData) {
      ...
      // Feature adjustments
      if (
        valuationData.vehicle_features &&
        valuationData.vehicle_features.length > 0
      ) {
        const featureAdjustments = valuationData.vehicle_features
          ...
          .map((vf) => ({
            ...
            description:
              `Premium feature: ${vf.features.name} (${vf.features.category})`,
          }));
      ...
      if (
        valuationData.photo_condition_scores &&
        valuationData.photo_condition_scores.length > 0
      ) {
      ...
      const userBehavior = {
        ...
        pending_offers: valuationData.dealer_offers
          ? valuationData.dealer_offers.length
          : 0,
      ...
      const contextObj = {
        ...
        valuation: {
          ...
          features: valuationData.vehicle_features
            ? valuationData.vehicle_features.map((vf) => vf.features?.name)
              .filter(Boolean)
            : [],
        },
      ...
      if (!userBehavior.has_uploaded_photos) {
        suggestions.push(
          "The user hasn't uploaded photos yet. Consider suggesting that uploading clear car photos could improve their valuation accuracy and potentially increase their value.",
        );
      }

      if (userBehavior.pending_offers > 0) {
        suggestions.push(
          `The user has ${userBehavior.pending_offers} dealer offer(s). You can offer to explain these offers or help them understand how to respond.`,
        );
      }

      if (!userBehavior.premium_user && valuationData.estimated_value > 10000) {
        suggestions.push(
          "This user hasn't unlocked premium features yet. For valuable vehicles, consider mentioning how the premium report could help them negotiate a better deal.",
        );
      }
      ...
      messages.push({
        role: "system",
        content:
          "No specific valuation data is available for this conversation. Provide general guidance about car valuations based on industry knowledge.",
      });
    }
    ...
    if (responseError) {
      throw new Error(
        `Failed to store assistant response: ${responseError.message}`,
      );
    }
