...
class HemmingsProvider(ListingProvider):
    def fetch_listings(self, max_results=100):
        url = "https://www.hemmings.com/cars-for-sale/"
        listings = []
        resp = requests.get(url, headers={"User-Agent": "Mozilla/5.0"})
        soup = BeautifulSoup(resp.text, "html.parser")
        for card in soup.select("div.listing-main-info"):
            title = card.find("a", class_="listing-title-link").text if card.find("a", class_="listing-title-link") else None
            price = card.find("span", class_="listing-price").text if card.find("span", class_="listing-price") else None
            url_tag = card.find("a", class_="listing-title-link", href=True)
            url_listing = f"https://www.hemmings.com{url_tag['href']}" if url_tag else None
