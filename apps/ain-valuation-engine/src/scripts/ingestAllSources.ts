...
import { DATA_SOURCES, DataSourceMeta, NormalizedVehicleListing } from '../data/autoDataSources';
import axios from 'axios';

// Example: Ingestion handler for a single source (API or scraping)
async function ingestSource(source: DataSourceMeta): Promise<NormalizedVehicleListing[]> {
  ...
  if (source.supportsApi) {
    ...
    try {
      ...
      return (resp.data.listings || []).map((item: any) => ({
...
async function runIngestionPhase(tier?: string) {
