export function carsDotCom({ make, model, zip, radius = 100 }) {
...
export function carGurus({ make, model, zip }) {
...
export function autoTrader({ make, model, zip, radius = 100 }) {
    const m = encodeURIComponent(make ?? "");
    const mo = encodeURIComponent(model ?? "");
    const z = encodeURIComponent(zip ?? "");
    return `https://www.autotrader.com/cars-for-sale/all-cars/${m}/${mo}/${z}?dma=&searchRadius=${radius}`;
}
export function trueCar({ make, model, zip, radius = 100 }) {
...
export function edmunds({ make, model, zip, radius = 100 }) {
    const m = encodeURIComponent(make ?? "");
    const mo = encodeURIComponent(model ?? "");
    const z = encodeURIComponent(zip ?? "");
