...
export async function openaiFetchListings(
  promptOrKit: string | { system: string; example: string; schema: any; prompt: string }
) {
  ...
  let parsed;
  try { parsed = JSON.parse(text); } catch { parsed = { listings: [] }; }
  if (!parsed?.listings || !Array.isArray(parsed.listings)) return { listings: [], tokensIn: res.usage?.input_tokens || 0, tokensOut: res.usage?.output_tokens || 0 };
