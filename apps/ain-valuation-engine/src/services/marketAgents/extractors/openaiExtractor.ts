...
export type ExtractResult = { listings: MarketListing[]; tokensIn?: number; tokensOut?: number };
...
export async function extractListingsFromHtml(params: {
  ...
  let data: any = {};
  try { data = JSON.parse(resp.choices[0]?.message?.content ?? "{}") } catch { data = { listings: [] }; }
  const listings: MarketListing[] = Array.isArray(data.listings) ? data.listings : [];
  ...
  const tokensIn: number | undefined = (resp as any)?.usage?.input_tokens ?? (resp as any)?.usage?.prompt_tokens;
  const tokensOut: number | undefined = (resp as any)?.usage?.output_tokens ?? (resp as any)?.usage?.completion_tokens;
