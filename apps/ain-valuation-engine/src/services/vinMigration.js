 ...
 */
import { decodeVin, extractLegacyVehicleInfo } from './unifiedVinDecoder';
/**
 ...
 */
export async function legacyDecodeVin(vin) {
    try {
        const result = await decodeVin(vin);
        ...
        return {
            ...
            errorText: ((error?.message ?? String(error)) || 'VIN decoding failed')
        };
    }
}
/**
 ...
 */
export async function enhancedDecodeVin(vin) {
    try {
...
export async function testMigration() {
    ...
    try {
        ...
        console.error('\n❌ Migration test failed:', error?.message ?? String(error));
        return false;
    }
}
