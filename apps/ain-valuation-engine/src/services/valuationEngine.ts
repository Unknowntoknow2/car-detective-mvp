...
export class ValuationEngine {
  private readonly minComparables = 100;
  private readonly maxComparables = 200;
  private readonly searchRadius = 150; // miles
  private readonly inflationRate = 0.025; // 2.5% annual inflation

  async generateValuation(vehicleData: VehicleData): Promise<ApiResponse<ValuationResult>> {
    try {
  ...
  private async gatherMarketData(vehicleData: VehicleData): Promise<MarketListing[]> {
    console.log('📊 Gathering market data...');
    
    const marketResponse = await marketListingsService.getMarketListings(
      vehicleData,
      this.searchRadius,
      this.maxComparables
    );
  ...
  private async findComparables(vehicleData: VehicleData, marketData: MarketListing[]): Promise<MarketListing[]> {
  ...
  private calculateSimilarityScore(target: VehicleData, listing: MarketListing): number {
    let score = 0;
    let maxScore = 0;
    ...
    if (target.mileage && listing.mileage) {
      ...
      if (mileageDiff < 5000) score += 15;
      else if (mileageDiff < 10000) score += 12;
      else if (mileageDiff < 20000) score += 8;
      else if (mileageDiff < 40000) score += 4;
    }
    ...
    if (target.condition === listing.condition) {
      ...
      const conditionDiff = Math.abs(
        this.getConditionValue(target.condition) - this.getConditionValue(listing.condition)
      );
      if (conditionDiff === 1) score += 7;
      else if (conditionDiff === 2) score += 4;
    }
  ...
  private getConditionValue(condition: VehicleCondition): number {
    const values = {
  ...
  private calculateBaseValue(comparables: MarketListing[]): number {
    ...
    comparables.forEach(comp => {
      ...
      if (daysOld < 7) weight *= 1.3;
      else if (daysOld < 14) weight *= 1.1;
      else if (daysOld > 60) weight *= 0.8;

      // Dealer listings get slightly more weight due to standardized pricing
      if (comp.dealer) weight *= 1.1;
    ...
    const filteredComparables = comparables.filter(c => 
      c.price >= lowerBound && c.price <= upperBound
    );
  ...
  private calculateMileageAdjustment(mileageDiff: number): number {
  ...
  private calculateConditionAdjustment(condition: VehicleCondition): number {
    const adjustments = {
  ...
  private calculateTitleAdjustment(titleStatus: TitleStatus): number {
    const adjustments = {
      [TitleStatus.CLEAN]: 0,
  ...
  private calculateHistoryAdjustments(historyData: any): ValuationAdjustment[] {
    const adjustments: ValuationAdjustment[] = [];

    // Accident history adjustment
    if (historyData.accidentHistory?.length > 0) {
      const severityAdjustment = historyData.accidentHistory.reduce((total: number, accident: any) => {
    ...
    if (historyData.serviceRecords?.length > 0) {
      const recentService = historyData.serviceRecords.filter((record: any) => {
  ...
  private async calculateMarketFactors(vehicleData: VehicleData): Promise<MarketFactor[]> {
  ...
  private getSeasonalFactor(vehicleData: VehicleData, month: number): number {
    ...
    if (month >= 3 && month <= 8) { // Spring/Summer
      if (vehicleData.model.toLowerCase().includes('convertible')) return 0.05;
      if (vehicleData.drivetrain === 'awd' || vehicleData.drivetrain === '4wd') return -0.02;
    } else { // Fall/Winter
      if (vehicleData.model.toLowerCase().includes('convertible')) return -0.05;
      if (vehicleData.drivetrain === 'awd' || vehicleData.drivetrain === '4wd') return 0.03;
    }

    return 0;
  }

  private getSeasonalDescription(month: number, factor: number): string {
...
Please provide a concise explanation of how this valuation was calculated and what factors influenced the final price. Keep it under 200 words and make it understandable to consumers.`;
  ...
  private generateFallbackExplanation(finalValue: number, adjustments: ValuationAdjustment[]): string {
  ...
  private calculateAccuracy(comparables: MarketListing[], finalValue: number): AccuracyMetrics {
  ...
  private calculateConfidence(accuracy: AccuracyMetrics, comparableCount: number): number {
  ...
  private calculatePriceRange(finalValue: number, confidence: number): { low: number; high: number } {
