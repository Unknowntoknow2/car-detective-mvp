...
export interface MarketValuationResult {
  ...
  comps: Array<{ source: string; price: number; mileage?: number | null; url?: string }>;
  commentary: string[];
  error?: string;
}
...
export async function marketValuationFromVin({ vin, zip, mileage, condition, radius = 50 }: {
  ...
  try {
    ...
    const comps = (compsRaw || []).map((l: any) => ({
    ...
    // Commentary
    const commentary = [];
    if (spread < 0.05) commentary.push("✅ Spread is tight (<5%), so valuation is stable.");
    else if (spread < 0.10) commentary.push("⚠️ Spread is moderate (<10%), some market variation.");
    else commentary.push("⚠️ Spread is wide (>10%), market is volatile or comps are inconsistent.");
    if (confidence > 0.9) commentary.push("✅ Confidence is high because multiple Tier 1 marketplaces agree.");
    else if (confidence > 0.7) commentary.push("⚠️ Confidence is moderate; check comps for outliers.");
    else commentary.push("⚠️ Confidence is low; few comps or high spread.");
    // Confidence comment
    let confidence_comment = "";
    if (confidence > 0.9) confidence_comment = "High confidence (multiple sources, tight spread)";
    else if (confidence > 0.7) confidence_comment = "Moderate confidence (some source agreement)";
    else confidence_comment = "Low confidence (few comps or wide spread)";
