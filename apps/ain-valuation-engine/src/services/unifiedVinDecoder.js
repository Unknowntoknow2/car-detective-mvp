...
export async function decodeVin(vin, options = {}) {
    ...
    if (!forceDirectAPI) {
        try {
            ...
            console.warn('⚠️ Supabase edge function failed, falling back to direct API:', error?.message ?? String(error));
    ...
    try {
        ...
        throw new VINDecodeError('VIN decoding failed on all endpoints', 'ALL_ENDPOINTS_FAILED', { originalError: error?.message ?? String(error) });
    }
}
/**
...
async function callSupabaseEdgeFunction(vin, timeout) {
    ...
    const SUPABASE_URL = (typeof window !== 'undefined' && window.ENV?.VITE_SUPABASE_URL) ||
    ...
    try {
        ...
        if (!response.ok) {
            ...
            throw new VINDecodeError(errorData.error || `HTTP ${response.status}`, 'EDGE_FUNCTION_ERROR', errorData);
        }
        ...
        throw new VINDecodeError('Edge function request failed', 'NETWORK_ERROR', error?.message ?? String(error));
    }
}
/**
    ...
    for (let attempt = 0; attempt <= retries; attempt++) {
        ...
        try {
            ...
            if (error instanceof VINDecodeError) {
                if (attempt === retries)
                    throw error;
                console.warn(`⚠️ Attempt ${attempt + 1} failed: ${error?.message ?? String(error)}, retrying...`);
                continue;
            }
            if (attempt === retries) {
                throw new VINDecodeError('NHTSA API request failed', 'NETWORK_ERROR', error?.message ?? String(error));
            }
            console.warn(`⚠️ Attempt ${attempt + 1} failed: ${error?.message ?? String(error)}, retrying...`);
            // Exponential backoff
 ...
 */
export function extractLegacyVehicleInfo(decodedData) {
 ...
 */
export function getVinField(decodedData, category, field) {
