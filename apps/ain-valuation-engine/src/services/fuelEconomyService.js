...
export async function fetchFuelEconomyByYearMakeModel(year, make, model) {
    try {
        ...
        if (!id)
            return null;
        const vehicleResponse = await ExternalApiService.makeExternalCall(`https://www.fueleconomy.gov/ws/rest/vehicle/${id}`, { headers: { Accept: 'application/json' } });
 ...
 */
export async function fetchAvgFuelCostUSD() {
    try {
