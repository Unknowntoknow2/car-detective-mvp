...
export async function basicVinExample() {
    ...
    try {
        ...
        if (error instanceof VINDecodeError) {
            console.error('❌ VIN Decode Error:', error?.message ?? String(error), '(Code:', error.code, ')');
        }
        else {
...
export async function fallbackExample() {
    ...
    try {
        ...
        console.log('❌ Direct API failed:', error?.message ?? String(error));
    }
    ...
    try {
        ...
        console.log('❌ Both endpoints failed:', error?.message ?? String(error));
        return null;
    }
}
/**
...
export async function batchVinExample() {
    ...
    const results = await Promise.allSettled(vins.map(async (vin) => {
        const validation = validateVIN(vin);
        if (!validation.valid) {
...
export async function errorHandlingExample() {
    ...
    for (const testCase of testCases) {
        ...
        try {
            ...
            if (error instanceof VINDecodeError) {
                console.log(`   Decode: ❌ ${error.code}: ${error?.message ?? String(error)}`);
            }
            else {
                console.log(`   Decode: ❌ Unexpected: ${error?.message ?? String(error)}`);
            }
        }
    }
}
