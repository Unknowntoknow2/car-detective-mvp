...
export function ValuationResultsDisplay({ valuation }: ValuationResultsDisplayProps) {
    ...
    <div className="max-w-4xl mx-auto p-6 space-y-6">
      ...
      <div className="bg-white rounded-lg shadow-lg p-6">
        ...
        <div className="mt-8 text-center">
          <div className="inline-block p-6 bg-blue-50 rounded-lg">
            ...
            <div className={`inline-block px-3 py-1 rounded-full text-sm font-medium mt-3 ${getConfidenceColor(valuation.confidence)}`}>
        ...
        <div className="mt-6 grid grid-cols-2 gap-4 text-sm">
          <div><strong>Mileage:</strong> {valuation.vehicleData.mileage?.toLocaleString() || 'N/A'}</div>
          <div><strong>Condition:</strong> {valuation.vehicleData.condition || 'N/A'}</div>
          <div><strong>Title Status:</strong> {valuation.vehicleData.titleStatus || 'N/A'}</div>
          <div><strong>Location:</strong> {valuation.vehicleData.zipCode || 'N/A'}</div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              {valuation.adjustments.map((adjustment, index) => (
                <div key={index} className="flex justify-between items-start p-3 bg-gray-50 rounded-md">
                  <div className="flex-1">
                    ...
                    <div className="text-sm text-gray-600">{adjustment.explanation}</div>
                  </div>
        ...
        <div className="bg-white rounded-lg shadow-lg p-6">
          ...
          {valuation.marketFactors.length > 0 ? (
            <div className="space-y-3">
              {valuation.marketFactors.map((factor, index) => (
                <div key={index} className="flex justify-between items-start p-3 bg-gray-50 rounded-md">
                  ...
                  <div className={`font-semibold ${getAdjustmentColor(factor.impact * 100)}`}>
      ...
      <div className="bg-white rounded-lg shadow-lg p-6">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          <div className="text-center p-4 bg-blue-50 rounded-lg">
            <div className="text-2xl font-bold text-blue-600">{valuation.accuracy.comparableCount}</div>
            <div className="text-sm text-gray-600">Comparable Vehicles</div>
          </div>
          <div className="text-center p-4 bg-green-50 rounded-lg">
            <div className="text-2xl font-bold text-green-600">{valuation.accuracy.dataQuality}%</div>
            <div className="text-sm text-gray-600">Data Quality</div>
          </div>
          <div className="text-center p-4 bg-purple-50 rounded-lg">
            <div className="text-2xl font-bold text-purple-600">{valuation.accuracy.marketCoverage}%</div>
            <div className="text-sm text-gray-600">Market Coverage</div>
          </div>
          <div className="text-center p-4 bg-yellow-50 rounded-lg">
            <div className="text-2xl font-bold text-yellow-600">±{valuation.accuracy.confidenceInterval}%</div>
      ...
      <div className="bg-white rounded-lg shadow-lg p-6">
        ...
        <div className="overflow-x-auto">
            ...
            <tbody>
              {valuation.comparables.slice(0, 10).map((comp, index) => (
                <tr key={index} className="border-b hover:bg-gray-50">
                  <td className="p-2">
                    <div className="font-medium">{comp.make} {comp.model}</div>
                  ...
                  <td className="p-2 capitalize">{comp.condition.replace('_', ' ')}</td>
          ...
          {valuation.comparables.length > 10 && (
            <div className="mt-4 text-center text-gray-500 text-sm">
              Showing 10 of {valuation.comparables.length} comparable vehicles
            </div>
          )}
        </div>
      </div>
      ...
      <div className="bg-gray-50 rounded-lg p-6 text-center text-sm text-gray-600">
        <p>
          Report generated on {formatDate(valuation.timestamp)} by the AIN Valuation Engine
        </p>
        <p className="mt-2">
