...
type FileInfo = {
  path: string; loc: number; exports?: string[];
  functions: { name: string; lines: string; summary?: string }[];
...
async function* walk(dir: string): AsyncGenerator<string> {
  ...
  for (const name of list) {
    ...
    if (stat.isDirectory()) { yield* walk(full); }
      ...
      if (Array.from(EXT).some(ext => full.endsWith(ext))) yield full;
    }
  }
}

async function scanFile(path: string): Promise<FileInfo> {
  ...
  for await (const lineText of rl) {
    ...
    for (const [k, re] of Object.entries(PATTERNS)) {
      if (re.test(lineText)) hits[k].push({ file: path, line: loc, text: lineText.trim().slice(0, 240) });
    }
  ...
  await rl.close(); await file.close();
...
function summarize(files: FileInfo[]): Index["summary"] {
  ...
  for (const f of files) {
    for (const [k, hits] of Object.entries(f.hits)) counts[k] += hits.length;
    const found = new Set<string>();
    for (const b of baseline) if ((f.hits[b]?.length ?? 0) > 0) found.add(b);
    coverage[f.path] = found.size;
  }
  return { counts, baselineCoverage: coverage };
}

async function writeArtifacts(idx: Index) {
  ...
  for (const f of idx.files) {
    ...
    if (f.functions.length) lines.push(`functions: ${f.functions.map(fn=>fn.name).join(", ")}`);
    for (const [k, arr] of Object.entries(f.hits)) {
      if (!arr.length) continue;
      if (!["fetch","openai","schema","dedupe","storage","retry","make","model","year","price","mileage","vin","photos","dealer","zip","url","valuation","env"].includes(k)) continue;
      lines.push(`- **${k}** (${arr.length})`);
      for (const h of arr.slice(0,5)) lines.push(`  - L${h.line}: ${h.text}`);
    }
    lines.push("");
  }
...
export default async function runAudit() {
  ...
  for (const base of SCAN_DIRS) {
    ...
    try { await fs.stat(dir); } catch { continue; }
  ...
  for (const f of files) {
    ...
    done++; progress(done, files.length, "Auditing files:");
  }
  const idx: Index = {
    repoRoot: ROOT,
