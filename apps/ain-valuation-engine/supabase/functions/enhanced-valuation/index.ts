...
// Additional types to remove `any`

// Shape of a row from current_market_intelligence view
interface MarketIntelligenceViewRow {
...
// Shape returned by the "market-signals" edge function
interface FreshMarketSignalsResponse {
  market_score?: number;
  market_temperature?: string;
  market_signals?: {
...
type MarketIntelFetchSource =
  | "market_intelligence_view"
  | "fresh_market_signals"
  | "default_fallback"
  | "error_fallback";

interface MarketIntelligenceFetchResult {
  data: MarketIntelligenceViewRow | FreshMarketSignalsResponse | null;
  fetch_time_ms: number;
  source: MarketIntelFetchSource;
  error?: string;
}

interface AppliedAdjusterDetails {
  description?: string;
  factor: number;
}

interface ApplyMarketAdjustersResult {
  final_valuation: number;
  applied_adjusters?: Record<string, AppliedAdjusterDetails>;
...
const baseValuationCache = new Map<
  string,
  { value: number; timestamp: number }
>();
const activeRequests = new Map<string, Promise<EnhancedValuationResponse>>();

// Base valuation calculation (simplified ML model simulation)
function calculateBaseValuation(
  vehicle: VehicleDetails,
  mileage: number,
  condition: EnhancedValuationRequest["condition"],
): number {
  ...
  const vehicleKey = `${make.toUpperCase()}_${
    model.toUpperCase().replace(/\s+/g, "_")
  }`;
  ...
  if (vehicleAge === 0) {
    ...
    const ageAdjustment = Math.max(
      0.2,
      1.0 - (depreciationRate * Math.pow(vehicleAge, 0.8)),
    );
  ...
  // Cache the result
  baseValuationCache.set(cacheKey, {
    value: finalValue,
    timestamp: Date.now(),
  });
...
async function fetchMarketIntelligence(
  ...
  model: string,
  region: string,
): Promise<MarketIntelligenceFetchResult> {
  console.log(
    `Fetching market intelligence for: ${year} ${make} ${model} in ${region}`,
  );
  ...
  try {
    ...
    if (intelligence && !intelligenceError) {
      const typedIntel = intelligence as MarketIntelligenceViewRow;
      console.log(
        `Found market intelligence from view: score ${typedIntel.composite_market_score}`,
      );
      return {
        data: typedIntel,
    ...
    console.log(
      "No cached intelligence found, fetching fresh market signals...",
    );

    const { data: marketSignals, error: signalsError } = await supabase
      .functions.invoke<FreshMarketSignalsResponse>("market-signals", {
        body: {
          year,
          make,
          model,
    ...
    if (marketSignals && !signalsError) {
      console.log(
        `Fresh market signals fetched: ${
          marketSignals.market_signals?.total_signals || 0
        } signals`,
      );
      return {
        data: marketSignals,
...
async function applyMarketAdjusters(
  ...
  model: string,
  region: string,
  valuationMode: EnhancedValuationRequest["valuation_mode"],
): Promise<ApplyMarketAdjustersResult | null> {
  try {
    const { data, error } = await supabase.rpc("apply_market_adjusters", {
    ...
    return data as ApplyMarketAdjustersResult;
  } catch (error) {
...
function generateExplanation(
  ...
  const adjustmentPercentage =
    ((finalValuation - baseValuation) / baseValuation) * 100;
  const adjustmentDirection = adjustmentPercentage >= 0
    ? "increased"
    : "decreased";
  const adjustmentMagnitude = Math.abs(adjustmentPercentage);

  const summary =
    `This ${vehicle.year} ${vehicle.make} ${vehicle.model} has been valued at $${finalValuation.toLocaleString()}, ` +
    `representing a ${
      adjustmentMagnitude.toFixed(1)
    }% ${adjustmentDirection} from the base valuation of $${baseValuation.toLocaleString()} ` +
    `due to current market conditions and regional factors.`;

  const keyFactors = [
    `Market Temperature: ${
      marketIntel?.market_temperature || "Unknown"
    } market conditions`,
    `Base Vehicle Value: $${baseValuation.toLocaleString()} before adjustments`,
    `Net Market Adjustment: ${adjustmentPercentage >= 0 ? "+" : ""}${
  ...
  if (marketIntel?.market_score) {
    marketInsights.push(
      `Market Score: ${marketIntel.market_score}/100 indicating ${marketIntel.market_temperature} market conditions`,
    );
  }
  if (marketIntel?.sales_momentum) {
    marketInsights.push(
      `Sales Momentum: ${marketIntel.sales_momentum} based on recent volume trends`,
    );
  }
  if (marketIntel?.consumer_interest) {
    marketInsights.push(
      `Consumer Interest: ${marketIntel.consumer_interest} search and engagement levels`,
    );
  }

  const adjusterBreakdown: string[] = [];
  if (adjusters?.market_condition) {
    adjusters.market_condition.forEach((adj: AdjusterDetails) => {
      const impact = adj.factor !== 1.0
        ? `${((adj.factor - 1.0) * 100).toFixed(1)}%`
        : "neutral";
      adjusterBreakdown.push(
        `${adj.name}: ${impact} impact (${adj.description})`,
      );
    });
  }
  ...
  return {
    ...
    market_insights: marketInsights.length > 0
      ? marketInsights
      : ["Market intelligence data not available"],
    adjuster_breakdown: adjusterBreakdown.length > 0
      ? adjusterBreakdown
      : ["No specific adjusters applied"],
  };
}

// Main enhanced valuation function
async function performEnhancedValuation(
  request: EnhancedValuationRequest,
): Promise<EnhancedValuationResponse> {
  const startTime = Date.now();
  const { year, make, model, mileage, condition, location, valuation_mode } =
    request;

  console.log(
    `Enhanced valuation request: ${year} ${make} ${model}, ${mileage} miles, ${condition} condition`,
  );
  ...
  const baseValuation = calculateBaseValuation(
    vehicle,
    mileage,
    condition,
  );
  ...
  if (request.include_market_intelligence !== false) {
    ...
    if (marketData.data) {
      if (marketData.source === "market_intelligence_view") {
        ...
        const intel = marketData.data as MarketIntelligenceViewRow;
        marketIntelligence = {
          market_score: intel.composite_market_score || 50,
          market_temperature: intel.market_temperature || "warm",
          sales_momentum: intel.avg_sales_volume && intel.avg_sales_volume > 15000
            ? "strong"
            : intel.avg_sales_volume && intel.avg_sales_volume < 8000
            ? "weak"
            : "moderate",
          price_trend: intel.avg_price ? "stable" : "unknown",
          consumer_interest: intel.search_volume && intel.search_volume > 70
            ? "high"
            : intel.search_volume && intel.search_volume < 40
            ? "low"
            : "moderate",
          market_liquidity: intel.avg_days_on_market && intel.avg_days_on_market < 30
            ? "high"
            : intel.avg_days_on_market && intel.avg_days_on_market > 50
            ? "low"
            : "moderate",
          regional_factors: intel.market_insights || [],
          data_quality: intel.intelligence_quality_score || 0.7,
          last_updated: intel.latest_signal_date ||
            new Date().toISOString().split("T")[0],
        };
      } else if (marketData.source === "fresh_market_signals") {
        // Data from fresh market signals
        const signals = marketData.data as FreshMarketSignalsResponse;
        marketIntelligence = {
  ...
  const adjustmentBreakdown: AdjustmentBreakdown = {
    market_condition: [],
    regional_factors: [],
    consumer_behavior: [],
    seasonal_patterns: [],
  ...
  if (adjustersResult) {
    ...
    adjustersApplied =
      Object.keys(adjustersResult.applied_adjusters || {}).length;

    // Parse adjuster details from RPC result
    if (adjustersResult.applied_adjusters) {
      Object.entries(adjustersResult.applied_adjusters).forEach(
        ([type, details]: [string, AppliedAdjusterDetails]) => {
          const factor = typeof details.factor === "number" ? details.factor : 1.0;
          const name = details.description || type;
          const adjuster: AdjusterDetails = {
            type,
            name,
            factor,
            impact_amount: (factor - 1.0) * baseValuation,
  ...
  const valuationRecord = {
    ...
    market_condition_adjustments: {
      adjusters: adjustmentBreakdown.market_condition,
    },
    regional_adjustments: { adjusters: adjustmentBreakdown.regional_factors },
    consumer_behavior_adjustments: {
      adjusters: adjustmentBreakdown.consumer_behavior,
    },
    seasonal_adjustments: { adjusters: adjustmentBreakdown.seasonal_patterns },
    vehicle_specific_adjustments: {
      adjusters: adjustmentBreakdown.vehicle_specific,
    },
  ...
  return {
    ...
    processing_details: {
      ...
      data_sources_used: request.market_data_sources ||
        ["market_intelligence", "base_model"],
    },
  };
}
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (
      ...
      !request.condition || !request.location?.state
    ) {
      return new Response(
        JSON.stringify({
    ...
    const coalescingKey =
      `${request.year}_${request.make}_${request.model}_${request.mileage}_${request.condition}_${request.location.state}`;
