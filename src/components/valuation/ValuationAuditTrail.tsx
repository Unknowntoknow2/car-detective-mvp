...
export function ValuationAuditTrail(
  { auditTrail, photoUrl, aiCondition }: ValuationAuditTrailProps,
) {
        ...
        <div className="grid grid-cols-1 md:grid-cols-7 gap-4 items-start">
          {photoUrl && (
            <div className="md:col-span-2">
              ...
              {aiCondition && aiCondition.issuesDetected &&
          ...
          <div
            className={`space-y-4 ${
              photoUrl ? "md:col-span-5" : "md:col-span-7"
            }`}
          >
            <div>
              <p className="text-sm font-medium mb-1">Base Value</p>
              <p className="text-xl font-bold">
                {formatCurrency(auditTrail.basePrice)}
              </p>
            </div>
            ...
            <div>
              ...
              <ul className="space-y-2">
                {auditTrail.adjustments.map((adjustment, index) => (
                  <li
                    key={index}
                    className="grid grid-cols-2 text-sm border-b border-muted pb-2"
                  >
                    <div>
            ...
            <div className="pt-2 border-t border-muted">
              ...
              <p className="text-xs text-muted-foreground mt-1">
                Based on data from{" "}
                {new Date(auditTrail.timestamp).toLocaleDateString()}
              </p>
            </div>
          </div>
        </div>
