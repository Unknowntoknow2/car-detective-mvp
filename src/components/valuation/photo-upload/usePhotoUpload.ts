    ...
    const fetchPhotos = async () => {
      ...
      try {
        ...
        if (data && data.length > 0) {
          ...
          const highestScorePhoto = [...fetchedScores].sort((a, b) =>
            b.score - a.score
          )[0];
          if (highestScorePhoto) {
            const bestPhotoData = fetchedPhotos.find((p) =>
              p.url === highestScorePhoto.url
            );
            if (bestPhotoData) {
    ...
    try {
      ...
      for (const file of files) {
        ...
        if (file.size > MAX_FILE_SIZE) {
          console.warn(
            `Skipped file ${file.name}: Size too large (${
      ...
      if (!validFiles.length) {
        setError(
          "No valid files to upload. Please select JPEG or PNG images under 10MB.",
        );
      ...
      updateBestPhoto([...photoScores, ...newScores], [
        ...photos,
        ...uploadedPhotos,
      ]);
