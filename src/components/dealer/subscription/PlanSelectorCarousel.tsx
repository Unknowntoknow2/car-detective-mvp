...
export const PlanSelectorCarousel: React.FC<PlanSelectorCarouselProps> = ({
  ...
  const handleSelectPlan = (planId: string) => {
    ...
    setTimeout(() => {
      setIsLoading(false);
      toast({
        description: `You've successfully selected the ${
    ...
    <div className="w-full">
      <div className="flex justify-between items-center mb-6">
        ...
        <div className="flex gap-2">
          ...
          <Button
            ...
            size="icon"
            onClick={handleNext}
            disabled={visiblePlans[visiblePlans.length - 1] >=
        ...
        {availablePlans.map((plan, index) => {
          ...
          return (
            ...
            >
              ...
              >
                {isCurrent && (
                  <Badge className="absolute -top-2.5 left-1/2 transform -translate-x-1/2 bg-primary px-3 z-10">
                    Current Plan
                  </Badge>
                )}
                {plan.popular && (
                  <Badge className="absolute -top-2.5 right-6 bg-amber-500/90 px-3 z-10 flex items-center gap-1">
      ...
      <div className="flex justify-center mt-6">
        <div className="flex gap-1">
          {Array.from({
            length: Math.ceil(availablePlans.length / maxVisiblePlans),
          }).map((_, i) => {
