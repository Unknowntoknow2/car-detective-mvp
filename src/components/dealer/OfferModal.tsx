...
export function OfferModal(
  { isOpen, onClose, lead, onSubmit }: OfferModalProps,
) {
  ...
  const getSuggestedAmount = () => {
    if (lead.condition_score >= 85) {
      return Math.floor(lead.estimated_value * 0.95);
    }
    if (lead.condition_score >= 70) {
      return Math.floor(lead.estimated_value * 0.9);
    }
    if (lead.condition_score >= 50) {
      return Math.floor(lead.estimated_value * 0.85);
    }
