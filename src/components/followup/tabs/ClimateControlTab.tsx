...
interface ClimateControlTabProps {
  formData: FollowUpAnswers;
  updateFormData: (updates: Partial<FollowUpAnswers>) => void;
  baseValue: number;
}

export function ClimateControlTab({ formData, updateFormData, baseValue }: ClimateControlTabProps) {
  const climateFeatures = getFeaturesByCategory('climate');
  ...
  const categoryValue = calculateEnhancedFeatureValue(
    selectedFeatures.filter(id => climateFeatures.some(f => f.id === id)),
    baseValue
  );
