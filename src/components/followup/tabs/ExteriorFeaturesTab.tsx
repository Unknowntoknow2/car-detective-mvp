...
interface ExteriorFeaturesTabProps {
  formData: FollowUpAnswers;
  updateFormData: (updates: Partial<FollowUpAnswers>) => void;
  baseValue: number;
}

export function ExteriorFeaturesTab({ formData, updateFormData, baseValue }: ExteriorFeaturesTabProps) {
  const exteriorFeatures = getFeaturesByCategory('exterior');
  ...
  const categoryValue = calculateEnhancedFeatureValue(
    selectedFeatures.filter(id => exteriorFeatures.some(f => f.id === id)),
    baseValue
  );
