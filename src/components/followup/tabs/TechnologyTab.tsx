...
export function TechnologyTab({ formData, updateFormData, baseValue }: TechnologyTabProps) {
  const technologyFeatures = getFeaturesByCategory('technology');
  ...
  const categoryValue = calculateEnhancedFeatureValue(
    selectedFeatures.filter(id => technologyFeatures.some(f => f.id === id)),
    baseValue
  );
