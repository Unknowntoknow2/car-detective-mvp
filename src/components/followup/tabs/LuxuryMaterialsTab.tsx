...
export function LuxuryMaterialsTab({ formData, updateFormData, baseValue }: LuxuryMaterialsTabProps) {
  const luxuryFeatures = getFeaturesByCategory('luxury_materials');
  ...
  const categoryValue = calculateEnhancedFeatureValue(
    selectedFeatures.filter(id => luxuryFeatures.some(f => f.id === id)),
    baseValue
  );
