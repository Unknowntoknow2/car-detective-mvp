...
interface PerformancePackagesTabProps {
  formData: FollowUpAnswers;
  updateFormData: (updates: Partial<FollowUpAnswers>) => void;
  baseValue: number;
}

export function PerformancePackagesTab({ formData, updateFormData, baseValue }: PerformancePackagesTabProps) {
  const performanceFeatures = getFeaturesByCategory('performance_packages');
  ...
  const categoryValue = calculateEnhancedFeatureValue(
    selectedFeatures.filter(id => performanceFeatures.some(f => f.id === id)),
    baseValue
  );
