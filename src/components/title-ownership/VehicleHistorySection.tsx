...
export function VehicleHistorySection() {
  ...
  const fetchVehicleHistory = async () => {
    ...
    try {
      ...
      if (data) {
        // We have data for this vehicle
        setVehicleData(data);
        setTitleStatus(data.title_brand || 'Clean');
        setNumberOfOwners(data.num_owners || 1);
        setHasFullServiceHistory(data.has_full_service_history || false);
