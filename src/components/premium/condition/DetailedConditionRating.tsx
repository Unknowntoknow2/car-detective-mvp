...
export function DetailedConditionRating(
  { onConditionChange, initialValues = {} }: DetailedConditionRatingProps,
) {
  ...
  const conditionTips: Record<string, Record<string, string>> = {
    exterior: {
      poor:
        "Paint is faded/damaged with visible rust/dents. Mismatched body panels or previous accident damage.",
      fair:
        "Some scratches, minor dents, or fading. Glass may have chips. Trim may be faded.",
      good:
        "Minor imperfections in paint. No significant dents or damage. Trim and glass in good condition.",
      excellent:
        "Paint has deep shine with no significant scratches. No dents or body damage. All trim intact and like-new.",
    },
    interior: {
      poor:
        "Significant wear on seats, carpet stains, dash cracks, non-functioning electronics.",
      fair:
        "Noticeable wear on high-touch areas. Some stains on carpet or seats. Minor dash wear.",
      good:
        "Minimal wear on seats and carpet. All electronics function. Minor wear on steering wheel.",
      excellent:
        "Interior looks nearly new. No significant wear on seats, dash, or carpet. Like-new appearance.",
    },
    mechanical: {
      poor:
        "Noticeable mechanical issues. May have irregular engine sound, rough shifting, or braking issues.",
      fair:
        "Engine runs, but may need minor maintenance. Some normal wear on mechanical components.",
      good:
        "Engine and transmission operate smoothly. Brakes and suspension in good condition.",
      excellent:
        "All mechanical systems function flawlessly. Recent service records. No issues during operation.",
    },
    maintenance: {
      poor:
        "Limited or no service records. Overdue for multiple maintenance items.",
      fair:
        "Some maintenance records. May be due for major service items like timing belt.",
      good:
        "Regular maintenance performed, though some items may be coming due.",
      excellent:
        "Complete service history. All maintenance up to date. Recent major services completed.",
    },
  };
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        ...
        <Badge className={`px-3 py-1 ${getConditionStyle(overallCondition)}`}>
          {overallConditionLabel.charAt(0).toUpperCase() +
            overallConditionLabel.slice(1)} ({Math.round(overallCondition)}%)
        </Badge>
      </div>
        ...
        {categories.map((category) => (
          <Card
            ...
            className={`border-l-4 ${
              ratings[category.id] === ratings[activeTipCategory] &&
            ...
            <CardContent className="p-4">
              <div className="flex justify-between items-center mb-2">
                ...
                <Badge className={getConditionStyle(ratings[category.id])}>
                  {getConditionLabel(ratings[category.id]).charAt(0)
                    .toUpperCase() +
              ...
              <div className="pt-2 pb-4">
                <Slider
                  ...
                  step={5}
                  onValueChange={(value) =>
                    handleSliderChange(value, category.id)}
      ...
      <div className="mt-4">
        <ConditionTips
          category={activeTipCategory.charAt(0).toUpperCase() +
            activeTipCategory.slice(1)}
          tip={getCurrentTip()}
        />
      </div>
    </div>
