...
export const LeadsSidebar: React.FC<LeadsSidebarProps> = ({ onSelectLead }) => {
  ...
  const filteredLeads = leads.filter((lead) => {
    ...
    if (selectedTab === "unread") {
      return !lead.isArchived && lead.unreadCount > 0;
    }
      ...
      <div className="h-full flex items-center justify-center p-6">
        <div className="text-center">
          ...
          {selectedTab === "archived" && (
            <p className="text-sm text-gray-500">No archived leads</p>
          )}
        </div>
      </div>
    );
        ...
        <div className="flex-1 min-w-0">
          <div className="flex justify-between items-baseline">
            <h3
              className={`font-medium text-sm truncate ${
                lead.unreadCount > 0 ? "text-black" : "text-gray-700"
              }`}
            >
              {lead.buyerName}
            </h3>
          ...
          <div className="flex items-center gap-1 mb-1">
            <span className="text-xs text-gray-500 truncate">
              {lead.vehicleInfo.year} {lead.vehicleInfo.make}{" "}
          ...
          <p
            className={`text-sm truncate ${
              lead.unreadCount > 0 ? "font-medium text-black" : "text-gray-600"
            }`}
          >
            {lead.lastMessage.sentByDealer && "You: "}
        ...
        {isHovered && (
          ...
          >
            ...
            >
              <Pin
                size={14}
                className={lead.isPinned ? "text-yellow-500" : ""}
              />
            </Button>
