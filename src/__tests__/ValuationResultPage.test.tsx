...
const renderComponent = (
  ...
  jest.spyOn(pdfModule, "generateValuationPdf").mockResolvedValue(
    Buffer.from([1, 2, 3]),
  );
...
describe("ValuationResultPage", () => {
  ...
  test("renders valuation result and AI chat bubble when data is available", async () => {
    ...
    expect(screen.getByTestId("ai-chat-bubble")).toHaveTextContent(
      "Toyota Camry",
    );
  });
  ...
  test("does not render PDF download button when valuation is not premium", async () => {
    ...
    await waitFor(() => {
      expect(screen.queryByTestId("pdf-download-button")).not
        .toBeInTheDocument();
    });
  });
  ...
  test("shows error message when there is an error", () => {
    ...
    expect(screen.getByText(/could not load the valuation details/i))
      .toBeInTheDocument();
  });
