...
describe("DealerVehicleForm", () => {
  ...
  it("renders the form with all required fields", () => {
    ...
    expect(screen.getByRole("button", { name: /add vehicle|submit|save/i }))
      .toBeInTheDocument();
  });
  ...
  it("validates required fields on submission", async () => {
    ...
    await user.click(
      screen.getByRole("button", { name: /add vehicle|submit|save/i }),
    );

    // Check for validation errors
    await waitFor(() => {
      expect(screen.getAllByText(/required|is required/i).length)
        .toBeGreaterThan(0);
    });
  ...
  it("prefills form data in edit mode", () => {
    ...
    expect(screen.getByRole("button", { name: /update|save changes/i }))
      .toBeInTheDocument();
  });
