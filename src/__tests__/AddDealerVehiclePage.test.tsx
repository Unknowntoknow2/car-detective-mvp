...
describe("AddDealerVehiclePage", () => {
  ...
  test("renders all form fields correctly", () => {
    ...
    // Check for buttons
    expect(screen.getByRole("button", { name: /submit|save|add vehicle/i }))
      .toBeInTheDocument();
  ...
  test("displays validation errors when submitting empty form", async () => {
    ...
    const submitButton = screen.getByRole("button", {
      name: /submit|save|add vehicle/i,
    });
  ...
  test("displays validation error for invalid VIN", async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText(/vin must be 17 characters/i))
        .toBeInTheDocument();
    });
  });
  ...
  test("successfully submits form with valid data", async () => {
    ...
    fireEvent.change(screen.getByLabelText(/make/i), {
      target: { value: sampleVehicleData.make },
    });
    fireEvent.change(screen.getByLabelText(/model/i), {
      target: { value: sampleVehicleData.model },
    });
    fireEvent.change(screen.getByLabelText(/year/i), {
      target: { value: sampleVehicleData.year },
    });
    fireEvent.change(screen.getByLabelText(/price/i), {
      target: { value: sampleVehicleData.price },
    });
    fireEvent.change(screen.getByLabelText(/mileage/i), {
      target: { value: sampleVehicleData.mileage },
    });
    fireEvent.change(screen.getByLabelText(/vin/i), {
      target: { value: sampleVehicleData.vin },
    });

    // For a select input like condition
    fireEvent.change(screen.getByLabelText(/condition/i), {
      target: { value: sampleVehicleData.condition },
    });
