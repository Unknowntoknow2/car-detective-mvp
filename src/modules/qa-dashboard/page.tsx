...
=======
  ...
  const handleViewResult = (id: string) => {
    globalThis.open(`/valuation/${id}`, "_blank");
  };
      ...
      <header className={styles.header}>
        <div>
          ...
          <p className="text-neutral-dark">
            Monitor and validate all system-critical flows
          </p>
        </div>
          ...
          <div className="flex items-center gap-4">
            <ProgressRing
              value={isLoadingMetrics
                ? 0
                : healthMetrics?.gptExplanationRate || 0}
              size={70}
              strokeWidth={6}
              color="#3B82F6"
              duration={1}
            >
              <span className="text-lg font-bold">
                {isLoadingMetrics
                  ? "-"
                  : `${Math.round(healthMetrics?.gptExplanationRate || 0)}%`}
              </span>
            </ProgressRing>
            <div className="text-sm">
              <div className="font-medium">Target: 100%</div>
              <div className="text-neutral-dark">
                All valuations have GPT explanations
              </div>
            </div>
          </div>
        </CDCard>
        ...
        <CDCard className={styles.stats.card}>
          ...
          <div className="flex items-center gap-4">
            <ProgressRing
              value={isLoadingMetrics
                ? 0
                : healthMetrics?.photoScoringRate || 0}
              size={70}
              strokeWidth={6}
              color="#10B981"
              duration={1.2}
            >
              <span className="text-lg font-bold">
                {isLoadingMetrics
                  ? "-"
                  : `${Math.round(healthMetrics?.photoScoringRate || 0)}%`}
              </span>
            </ProgressRing>
            <div className="text-sm">
          ...
          <div className="flex items-center gap-4">
            ...
            >
              <span className="text-lg font-bold">
                {isLoadingMetrics
                  ? "-"
                  : `${Math.round(healthMetrics?.pdfSuccessRate || 0)}%`}
              </span>
            </ProgressRing>
            <div className="text-sm">
        ...
        <CDCard className={styles.stats.card}>
          ...
          <div className="flex items-center gap-4">
            <ProgressRing
              value={isLoadingMetrics
                ? 0
                : healthMetrics?.paymentMatchRate || 0}
              size={70}
              strokeWidth={6}
              color="#6366F1"
              duration={1.6}
            >
              <span className="text-lg font-bold">
                {isLoadingMetrics
                  ? "-"
                  : `${Math.round(healthMetrics?.paymentMatchRate || 0)}%`}
              </span>
            </ProgressRing>
            <div className="text-sm">
              <div className="font-medium">Target: 100%</div>
              <div className="text-neutral-dark">
                Premium access payment matches
              </div>
            </div>
          </div>
        </CDCard>
      ...
      <div className={styles.filterBar}>
        ...
        <div className="w-full sm:w-auto flex-1">
          <label className="text-sm font-medium block mb-2">
            Confidence Score: {confidenceRange[0]}% - {confidenceRange[1]}%
          </label>
          <Slider
