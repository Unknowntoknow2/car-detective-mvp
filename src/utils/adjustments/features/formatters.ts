...
export function standardizeFeatureNames(rawFeatures: string[]): string[] {
  return rawFeatures.map((feature) => {
    ...
    if (
      normalizedFeature.includes("sun") && normalizedFeature.includes("roof")
    ) return "sunroof";
    if (
      normalizedFeature.includes("moon") && normalizedFeature.includes("roof")
    ) return "sunroof";
    if (
      normalizedFeature.includes("pano") && normalizedFeature.includes("roof")
    ) return "panoramic roof";
    if (normalizedFeature.includes("nav")) return "navigation system";
    if (
      normalizedFeature.includes("premium") &&
      normalizedFeature.includes("sound")
    ) return "premium audio";
    if (
      normalizedFeature.includes("premium") &&
      normalizedFeature.includes("audio")
    ) return "premium audio";
    if (
      normalizedFeature.includes("heated") && normalizedFeature.includes("seat")
    ) return "heated seats";
    if (
      normalizedFeature.includes("cool") && normalizedFeature.includes("seat")
    ) return "ventilated seats";
    if (
      normalizedFeature.includes("vent") && normalizedFeature.includes("seat")
    ) return "ventilated seats";
