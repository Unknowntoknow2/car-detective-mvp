...
export function calculateFinalValuation(
  ...
  if (params.baseMarketValue === undefined || params.baseMarketValue <= 0) {
    throw new Error(
      "Base market value is required and must be greater than zero",
    );
  }
...
=======
  if (params.features && params.features.length > 0) {
    ...
    adjustments.push({
      ...
      description: `${params.features.length} premium features including ${
        params.features.slice(0, 2).join(", ")
      }${params.features.length > 2 ? "..." : ""}`,
  ...
  if (params.make && params.model && (params.year || params.vehicleYear)) {
    ...
    const trendImpact = calculateMakeModelTrend(
      params.make,
      params.model,
      year,
      baseValue,
    ...
    if (trendImpact !== 0) {
      adjustments.push({
        name: "Market Trends",
        description:
          `Current market trends for ${year} ${params.make} ${params.model}`,
        impact: trendImpact,
...
function getRegionalMarketDescription(
  zipCode: string,
  multiplier: number,
): string {
  if (multiplier > 0.03) {
...
function calculateMakeModelTrend(
  make: string,
  model: string,
  year: number,
  baseValue: number,
