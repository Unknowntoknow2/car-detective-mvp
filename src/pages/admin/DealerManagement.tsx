...
export default function DealerManagement() {
  ...
  const filteredDealers = searchTerm
    ? dealers.filter((dealer) =>
      dealer.dealership_name?.toLowerCase().includes(
        ...
        <div className="md:col-span-2">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Manage dealer accounts and premium status
              </CardDescription>
                        ...
                        {filteredDealers.length === 0
                          ...
                          : (
                            filteredDealers.map((dealer) => (
                              <TableRow
                                key={dealer.id}
                                className={selectedDealer === dealer.id
                                  ? "bg-muted"
                                  : undefined}
                              >
        ...
        <div>
          {selectedDealer && (
            <PremiumDealerToggle
              ...
              isCurrentlyPremium={dealers.find((d) => d.id === selectedDealer)
                ?.is_premium_dealer || false}
              dealerName={dealers.find((d) => d.id === selectedDealer)
                ?.dealership_name || "Unnamed Dealership"}
              expiresAt={dealers.find((d) => d.id === selectedDealer)
                ?.premium_expires_at}
