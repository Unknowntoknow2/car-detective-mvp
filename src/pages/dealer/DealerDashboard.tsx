...
import DealerDashboardContent from '@/components/dealer/DealerDashboardContent';
  ...
  >
    <CardHeader className="bg-primary/5 pb-2">
      <div className="flex items-center justify-between">
        ...
        <div className="rounded-full bg-primary/10 p-2 transition-transform hover:rotate-3">
          {icon}
        </div>
      </div>
    </CardHeader>
  ...
  useEffect(() => {
    const fetchDealerStats = async () => {
      try {
        ...
        const { count: valuationsCompleted, error: valuationsError } =
          await supabase
            .from("valuations")
            .select("*", { count: "exact", head: true })
            .eq("user_id", user.id);
        ...
        const averagePrice = priceData.length
          ? priceData.reduce((acc, vehicle) => acc + (vehicle.price || 0), 0) /
            priceData.length
          : null;
              ...
              ? (
                <div className="flex flex-col">
                  <span className="text-xl font-semibold">
                    {stats.lastAddedVehicle.year} {stats.lastAddedVehicle.make}
                    {" "}
                    {stats.lastAddedVehicle.model}
                  </span>
                  <span className="text-xs text-muted-foreground mt-1">
                    {format(
                      new Date(stats.lastAddedVehicle.created_at),
                      "MMM d, yyyy",
                    )}
                  </span>
                </div>
