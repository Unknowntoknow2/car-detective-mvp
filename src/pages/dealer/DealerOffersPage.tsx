...
export default function DealerOffersPage() {
  ...
  const [valuations, setValuations] = useState<
    Array<{ id: string; make: string; model: string; year: number }>
  >([]);
      ...
      try {
        ...
        if (await isDealerUser()) {
          ...
          const uniqueReportIds = [
            ...new Set(data.map((item) => item.report_id)),
          ];
          ...
          ? (
            <Card className="p-6">
              <CardTitle className="mb-4">
                No Vehicle Valuations Found
              </CardTitle>
              <CardContent className="pt-0">
                <p className="text-muted-foreground">
                  You don't have any vehicle valuations in the system. Create a
                  valuation first to receive dealer offers.
                </p>
              </CardContent>
            </Card>
          )
              ...
              {valuations.map((valuation) => (
                <TabsContent key={valuation.id} value={valuation.id}>
                  <Card>
                    <CardHeader>
                      <CardTitle>
                        Offers for {valuation.year} {valuation.make}{" "}
                        {valuation.model}
                      </CardTitle>
                    </CardHeader>
                    <CardContent>
